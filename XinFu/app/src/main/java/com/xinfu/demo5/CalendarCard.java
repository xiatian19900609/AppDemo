...
public class CalendarCard extends View {
    ...
    private ArrayList<Long> mSignInDate;
    ...
    public CalendarCard(Context context, ArrayList<Long> signInDate, OnCellClickListener listener) {
        super(context);
        this.mSignInDate = signInDate;
        ...
        for (int j = 0; j < TOTAL_ROW; j++) {
            ...
            for (int i = 0; i < TOTAL_COL; i++) {
                ...
                if (position >= firstDayWeek
                    ...
                    // 签到日期
                    for (Long l : mSignInDate) {
                        String d = DateUtil.millis2String(l);
                        String[] s = d.split("-");
                        if (isCurrentMonth && day == Integer.parseInt(s[2])) {
        ...
        public void drawSelf(Canvas canvas) {
            switch (state) {
                case TODAY: // 今天
                    ...
                    canvas.drawCircle((float) (mCellSpace * (i + 0.55)),
                            (float) ((j + 0.52) * mCellRowSpace), circleRadius,
                            mCirclePaint);
                    break;
    ...
    public void update(ArrayList<Long> signInDate) {
        this.mSignInDate = signInDate;
        fillDate();
        invalidate();
    }
